...
create index streets_city_difficulty_idx on private.streets (city_id, difficulty);
...
grant select on public.countries, public.country_translations, public.cities, public.city_translations
...
as $$
  select jsonb_build_object(
    'countries', coalesce(jsonb_agg(country_row.country_json order by country_row.country_name), '[]'::jsonb)
  )
  from (
    select
      coalesce(ct.name, c.code) as country_name,
      jsonb_build_object(
        'code', c.code,
        'name', coalesce(ct.name, c.code),
        'cities', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              'id', city.id,
              ...
              'center', extensions.st_asgeojson(city.center, 6)::jsonb,
              'bounds', extensions.st_asgeojson(city.bounds, 6)::jsonb,
              'difficultyCounts', (
                select jsonb_object_agg(counts.difficulty::text, counts.street_count)
                from (
                  select d.difficulty, count(s.id) as street_count
                  from unnest(enum_range(null::public.difficulty)) d(difficulty)
                  left join private.streets s
                    on s.city_id = city.id and s.difficulty = d.difficulty
                  group by d.difficulty
                ) counts
              )
            ) order by coalesce(city_t.name, city.slug)
          )
          from public.cities city
          left join public.city_translations city_t
            on city_t.city_id = city.id and city_t.locale = p_locale
          where city.country_id = c.id and city.enabled
        ), '[]'::jsonb)
      ) as country_json
    from public.countries c
    left join public.country_translations ct
      on ct.country_id = c.id and ct.locale = p_locale
  ) country_row;
$$;
...
begin
  ...
  if (
    select count(*)
    from private.streets s
    where s.city_id = v_city_id and s.difficulty = p_difficulty
  ) < 10 then
    raise exception using
      errcode = 'P0001',
      message = 'This city and difficulty need at least ten target streets.';
